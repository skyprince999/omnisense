...
- [Scene Index](#scene-index)
    - [Open-weight models with Sandbox Compute](#open-weight-models-with-sandbox-compute)
...
```
from videodb import SandboxTier, SceneExtractionType
...
model_name = "google/gemma-4-31B-it-FP8" ,
prompt = "Describe the scene in a clear, concise way." ,
sandbox_id = sandbox.id,
)
```

VideoDB validates that the sandbox is active, belongs to your workspace/account, supports the requested model tier, and was created with a matching `models` or `model_categories` value when model selection was provided. See [Sandbox Compute](\pages\core-concepts\sandbox-compute) and [Sandbox Models](\pages\core-concepts\sandbox-models) for setup and supported model names.
...
[Indexes &amp; Search](\pages\understand\legacy-indexing-search\indexes-and-search) [Frame Processing Primitives](\pages\understand\legacy-indexing-search\multimodal-indexing)
